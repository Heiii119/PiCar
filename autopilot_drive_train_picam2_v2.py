...
class PiCam2Manager:
    def __init__(self, width=IMAGE_W, height=IMAGE_H, framerate=CAMERA_FRAMERATE,
        ...
        # Older-compatible API: only main stream, Qt-friendly format
...
def preview_and_record():
    ...
    # optional: arm ESC by holding stop PWM briefly (increase if your ESC needs more)
    ctrl.stop()
    time.sleep(2.5)
...
def _robust_load_model(path):
    # Prefer native .keras; if .h5 provided, load with compile=False to avoid legacy metric deserialization issues.
    try:
