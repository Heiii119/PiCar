...
# Control loop rate (Hz): how often your loop reads keys, captures frames, and updates PWM
DRIVE_LOOP_HZ = 20
MAX_LOOPS = None
...
# Tie camera framerate target to the control loop target
...
class MotorServoController:
    def __init__(self, config):
        ...
        # Set PCA9685 PWM frequency to 60 Hz (RC servo/ESC friendly)
...
class PiCam2Manager:
    def __init__(self, width=IMAGE_W, height=IMAGE_H, framerate=CAMERA_FRAMERATE,
        ...
        # Single-stream configuration; Qt-friendly format
...
def preview_and_record():
    ...
    # Control loop timing derived from DRIVE_LOOP_HZ
    period = 1.0 / DRIVE_LOOP_HZ
    last_loop = time.time()

    # Arm ESC by holding STOP; some ESCs need 2+ seconds (adjust if needed)
    ctrl.stop()
    time.sleep(1.0)
    ...
    try:
        with RawKeyboard() as global_kb:
            ...
            while True:
                ...
                # Maintain loop rate at DRIVE_LOOP_HZ
                now = time.time()
                dt = now - last_loop
                if dt < period:
