#!/usr/bin/env python3
"""
PiCar - ONNX Sign Detection + Line Following Autopilot
"""
...
class PiCar:
    ...
    def line_follow(self, frame):
        ...
        if M["m00"] != 0:
            ...
            steer = self.motors.center_pwm() + int(error * 0.6)
            steer = np.clip(
                steer,
...
@app.route("/")
def index():
    return "<h1>PiCar Running</h1><img src='/video'>"
